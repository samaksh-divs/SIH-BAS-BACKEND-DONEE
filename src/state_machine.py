...
class ExperimentStateMachine:
    ...
    def __init__(
        ...
        self.error_detector = ProceduralErrorDetector(cooldown_frames=error_cooldown_frames)

        # State machine variables
        self.current_index: int = 0
        self.status: str = "START"  # START, ACTIVE, WAITING, STEP_COMPLETED, ERROR, RECOVERY, COMPLETE, PAUSED, STOPPED
    ...
    def _load_sequence(self) -> None:
        if not os.path.exists(self.sequence_config_path):
            raise FileNotFoundError(f"Sequence config '{self.sequence_config_path}' not found.")
        ...
        with open(self.sequence_config_path, 'r', encoding='utf-8') as f:
            ...
            for item in seq:
                s_def = StateDefinition(
                    ...
                    human_readable_name=str(item.get("human_readable_name", item["action"])),
                    message=str(item.get("message", "")),
                    expected_objects=list(item.get("expected_objects", [])),
                    expected_interaction=str(item.get("expected_interaction", "")),
                    timeout_seconds=float(item.get("timeout_seconds", 30.0)),
                    grace_period_seconds=float(item.get("grace_period_seconds", 5.0)),
                    next_state=item.get("next_state")
                )
                self.states[s_def.state_id] = s_def
                self.step_list.append(s_def)
                self.action_to_step_index[s_def.action] = s_def.step_number - 1

    @property
    def current_state(self) -> StateDefinition:
        return self.step_list[self.current_index]

    def update(self, observed: ObservedAction, frame: int, timestamp: float) -> StateUpdate:
        """
        ...
        if self.is_stopped:
            return self._build_update(observed, frame, timestamp, transitioned=False, prev_state=previous_state_id, status="STOPPED", msg="Experiment stopped.")
        if self.is_paused:
            return self._build_update(observed, frame, timestamp, transitioned=False, prev_state=previous_state_id, status="PAUSED", msg="Experiment paused.")

        # 2. Check if already complete
        if self.current_state.state_id == "S16" or self.current_index >= len(self.step_list) - 1:
            self.status = "COMPLETE"
            return self._build_update(observed, frame, timestamp, transitioned=False, prev_state=previous_state_id, status="COMPLETE", msg="Experiment complete.")
        ...
        if elapsed > curr_def.timeout_seconds:
            ...
            msg = f"Inactivity alert: Expected '{curr_def.action}'. {curr_def.message}"
            return self._build_update(observed, frame, timestamp, transitioned=False, prev_state=previous_state_id, status=status, err_type=error_type, msg=msg)

        # 4. Check for low confidence / perception uncertainty
        if obs_action == "UNCERTAIN" or observed.confidence_level == "UNCERTAIN":
            error_type = ErrorCategory.PERCEPTION_UNCERTAIN
            status = "WAITING"
            msg = f"Perception uncertain. {curr_def.message}"
            return self._build_update(observed, frame, timestamp, transitioned=False, prev_state=previous_state_id, status=status, err_type=error_type, msg=msg)

        # 4. Check for correct action match
        if obs_action == curr_def.action:
            ...
            if self.match_counter >= self.required_confirmations:
                ...
                trans_type = "NORMAL" if (self.current_index - old_idx == 1) else "JUMP"
                ...
                return self._build_update(
                    observed, frame, timestamp, transitioned=True, prev_state=prev_state, status=status, msg=msg,
                    comp_state=comp_state, comp_step_num=comp_step_num, comp_action=comp_action,
                    trans_type=trans_type
                )
            else:
                msg = f"In progress: {curr_def.message} ({self.match_counter}/{self.required_confirmations})"
                return self._build_update(observed, frame, timestamp, transitioned=False, prev_state=previous_state_id, status=status, msg=msg)

        # 5. Check for procedural error
        ...
        if err_event:
            ...
            msg = err_event.message if not err_event.is_debounced else curr_def.message
            ...
            if target_idx is not None and target_idx > self.current_index:
                ...
                # Resynchronize if operator insists on new step persistently (30 frames ≈ ~3.5s at 8fps)
                if self.recovery_counter >= 30:
                    ...
                    self.state_entry_timestamp = timestamp
                    self.error_detector.reset()
                    msg = f"Resynchronized to step {self.current_state.step_number}: {self.current_state.message}"
                    return self._build_update(
                        observed, frame, timestamp, transitioned=True, prev_state=prev_state, status=status,
                        err_type="RECOVERY", msg=msg, comp_state=comp_state, comp_step_num=comp_step_num,
                        comp_action=comp_action, trans_type="RECOVERY"
                    )

            return self._build_update(observed, frame, timestamp, transitioned=False, prev_state=previous_state_id, status=status, err_type=error_type, msg=msg)

        # 6. Check for timeout / inaction
        elapsed = timestamp - self.state_entry_timestamp
        if elapsed > curr_def.timeout_seconds:
            status = "WAITING"
            error_type = ErrorCategory.INACTION
