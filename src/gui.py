...
class ExperimentGUI:
    ...
    def _queue_live_frame_callback(self, cv_image: Any, update: StateUpdate, frame_obj: NormalizedFrame, obs_action: ObservedAction, metrics: Dict[str, Any]) -> None:
        self.update_queue.put(("LIVE_FRAME", update, frame_obj, obs_action, metrics, cv_image))

    def _queue_complete_callback(self) -> None:
        self.update_queue.put(("COMPLETE", None, None, None, None, None))
    ...
    def _process_queue(self) -> None:
        try:
            while True:
                msg_type, update, frame_obj, obs_action, metrics, cv_image = self.update_queue.get_nowait()
                if msg_type in ("UPDATE", "LIVE_FRAME"):
                    self._apply_update(update, frame_obj, obs_action, metrics, cv_image)
    ...
    def _apply_update(self, update: StateUpdate, frame_obj: NormalizedFrame, obs_action: ObservedAction, metrics: Optional[Dict[str, Any]] = None, cv_image: Any = None) -> None:
        ...
        if cv_image is not None and HAS_PIL:
            try:
                ...
                # Color map for each object class
                CLASS_COLORS = {
                    "white_container": (200, 200, 200),
                    "red_box":         (60,  60,  220),
                    "yellow_box":      (30,  210, 210),
                    "plant":           (50,  180,  50),
                    "spray_bottle":    (220, 130,  30),
                    "person":          (180, 100, 220),
                }

                draw = cv_image.copy()
                orig_h, orig_w = draw.shape[:2]
                ...
                for obj in frame_obj.objects:
                    if len(obj.bbox) == 4:
                        ...
                        cv2.rectangle(draw, (x1, y1 - th - 6), (x1 + tw + 4, y1), color, -1)
                ...
                bar_h = 22
                bar = np.zeros((bar_h, orig_w, 3), dtype=np.uint8)
                summary = "Detected: " + (", ".join(detected_labels) if detected_labels else "nothing")
                cv2.putText(bar, summary, (4, 15),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 100), 1, cv2.LINE_AA)
                draw = np.vstack([draw, bar])

                # --- Render to Tkinter canvas ---
                frame_rgb = cv2.cvtColor(draw, cv2.COLOR_BGR2RGB)
                canvas_w = self.video_canvas.winfo_width() or 560
                canvas_h = self.video_canvas.winfo_height() or 220
                pil_img = Image.fromarray(frame_rgb)
                pil_img = pil_img.resize((canvas_w, canvas_h), Image.BILINEAR)
                self._tk_photo = ImageTk.PhotoImage(pil_img)
                self.video_canvas.delete("all")
                self.video_canvas.create_image(0, 0, anchor=tk.NW, image=self._tk_photo)
            except Exception:
                pass
