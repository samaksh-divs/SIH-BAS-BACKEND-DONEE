...
class LiveCameraManager:
    ...
    def start_camera(self) -> bool:
        ...
        if self.mock_camera:
            ...
            self.pipeline.reset()  # Always reset to S01 on fresh camera start
    ...
    def _capture_loop(self) -> None:
        ...
        while self.is_running and not self.is_stopped:
            ...
            # 3. Person 1 Inference (always runs for FPS telemetry)
            p1_start = time.time()
            ...
            # 4. Downstream Person 2 ExperimentPipeline — ONLY after START EXPERIMENT
            if not self.is_experiment_started:
                # Not started yet: skip Person 2, but still fire callback for UI/video display
                if self.on_frame_callback:
                    from src.state_machine import StateUpdate
                    from src.perception_types import NormalizedFrame
                    from src.action_inference import ObservedAction
                    import time as _time
                    dummy_update = self.pipeline.state_machine._build_update(
                        ObservedAction(
                            action="UNCERTAIN", confidence=0.0, confidence_level="UNCERTAIN",
                            evidence=[], start_frame=frame_num, end_frame=frame_num,
                            timestamp=capture_ts, involved_objects=[], wrist_side="none",
                            reason="Waiting for experiment start"
                        ),
                        frame_num, capture_ts,
                        transitioned=False, prev_state=self.pipeline.state_machine.current_state.state_id,
                        status="START", msg="Click START EXPERIMENT to begin."
                    )
                    metrics = {
                        "camera_fps": self.camera_fps, "person1_fps": self.person1_fps,
                        "processing_fps": 0.0, "avg_latency_ms": 0.0,
                        "p95_latency_ms": 0.0, "max_latency_ms": 0.0,
                        "dropped_frames": self.dropped_frames, "frames_captured": self.frames_captured,
                        "mock_camera": self.mock_camera, "camera_status": self.camera_status,
                    ...
                    norm_frame = Person1Adapter.parse_record(perception_record)
                    obs_action = ObservedAction(
                        action="UNCERTAIN", confidence=0.0, confidence_level="UNCERTAIN",
                        evidence=[], start_frame=frame_num, end_frame=frame_num,
                        timestamp=capture_ts, involved_objects=[], wrist_side="none",
                        reason="Waiting for experiment start"
                    )
                    self.on_frame_callback(cv_image, dummy_update, norm_frame, obs_action, metrics)
                continue
